...
<head>
  <style>
    body {
      background-image: url("https://img.freepik.com/vector-gratis/fondo-mancha-acuarela-color-pastel_52683-12281.jpg?w=2000");
    }
    </style>
...
<body>
        ...
        <button type="button" >click para añadir</button> <a href="citas.php" target="_blank">añadir</a>
...
$sql = "SELECT id, nombre, apellidos,correo,telefono,tratamiento,dia,hora FROM citas";
...
echo "<table>";
echo "<tr>";
echo "<th>id</th><th>nombre</th><th>apellidos</th><th>correo</th><th>telefono</th><th>tratamiento</th><th>dia</th><th>hora</th><th>Acciones</th>";
echo "</tr>";
  // output data of each row
  while($row = $result->fetch_assoc()) {
   // echo "id: ".$row["id"]."nombre: ".$row["nombre"]."apellidos".$row["apellidos"]."correo".$row["correo"]."telefono".$row["telefono"]."tratamiento".$row["tratamiento"]."dia".$row["dia"]."hora".$row["hora"]."<br>";
    echo"<tr>";
    echo "<td>".$row["id"]."</td><td>".$row["nombre"]."</td><td>".$row["apellidos"].$row["correo"]."</td><td>".$row["telefono"]."</td><td>".$row["tratamiento"]."</td><td>".$row["dia"]."</td><td>".$row["hora"]."</td><td><a href='modificarc.php'>Modificar</a></td><td><a href='eliminarc.php'>Eliminar</a></td>";
    echo"</tr>";
  }
  echo "</table>";
} else {
